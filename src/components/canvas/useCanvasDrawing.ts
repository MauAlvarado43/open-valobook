...
export function useCanvasDrawing(
  ...
  const handleMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (tool === 'ability' && selectedAbilityIcon) {
      ...
      if (selectedAbilitySubType === 'wall' || selectedAbilitySubType === 'path' || selectedAbilitySubType === 'curved-wall') {
        ...
        const length = def ? getMinDimension(def, isPath ? 'height' : 'width') || (isPath ? 400 : 300) : (isPath ? 400 : 300);
        const thickness = isPath ? (def ? getAbilityDimension(def, 'width') : 80) || 80 : (def ? getAbilityDimension(def, 'height') : 12) || 12;
        ...
        const def = getAbilityDefinition(selectedAbilityName || selectedAbilityIcon || '');
        newAbility.radius = (def ? getAbilityDimension(def, 'radius') : (selectedAbilitySubType === 'smoke' ? 60 : 80)) || (selectedAbilitySubType === 'smoke' ? 60 : 80);
      }
    ...
    const newShape: DrawingElement = {
      id: `element-${Date.now()}`,
      type: tool === 'pen' ? 'freehand' : tool === 'timer-path' ? 'timer-path' : (tool as DrawingElement['type']),
      x: adjustedPos.x,
      y: adjustedPos.y,
      color: currentColor,
      strokeWidth: 2,
  ...
  const handleMouseMove = () => {
    ...
    if (currentShape.type === 'ability') {
      ...
      if (tool === 'pen' || tool === 'timer-path') {
        ...
        if (radius > 5) {
          setCurrentShape({ ...drawingShape, radius, rotation: Math.atan2(dy, dx) * (180 / Math.PI) });
        } else {
  ...
  const handleMouseUp = () => {
    ...
    if (currentShape.type === 'ability' && (currentShape as AbilityPlacement).subType === 'guided-path') {
      setTool('select');
      setSelectedAbilityIcon(null);
