...
import { AbilityProperties } from './properties/AbilityProperties';
import { VisionConeProperties } from './properties/VisionConeProperties';
import { GraphicProperties } from './properties/GraphicProperties';
...
export function ElementPropertiesPanel() {
  ...
  const commonOpacity = selectedElements.every(
    (el) => 'opacity' in el && (el as DrawingElement).opacity === (firstElement as DrawingElement).opacity
  )
    ? (firstElement as DrawingElement).opacity ?? null
    : null;

  const commonFontSize = selectedElements.every(
    (el) => 'fontSize' in el && (el as DrawingElement).fontSize === (firstElement as DrawingElement).fontSize
  )
    ? (firstElement as DrawingElement).fontSize || null
    : null;

  // Handlers
  const handleUpdateMany = (updates: Partial<DrawingElement | AgentPlacement | AbilityPlacement>) => {
  ...
  const showDrawingProperties = selectedElements.some(el =>
    el.type !== 'agent' &&
    el.type !== 'ability' &&
    el.type !== 'image' &&
  ...
  const abilityElements = selectedElements.filter(el => el.type === 'ability') as AbilityPlacement[];
  ...
  return (
    <div className="absolute bottom-4 left-4 bg-gray-900 bg-opacity-95 rounded-lg shadow-2xl p-4 border border-gray-700 min-w-[300px] max-h-[80vh] overflow-y-auto custom-scrollbar">
      ...
      <div className="space-y-4">
        ...
        {hasDrawingElement && firstElement.type !== 'vision-cone' && firstElement.type !== 'timer-path' && (
          <div className="pt-2 border-t border-gray-800">
            ...
            <div className="flex gap-2">
              <input
                type="range" min="0" max="360" step="1" title="Rotate"
              ...
              />
              <input
                type="number" min="0" max="360" title="Rotation value"
                value={(firstElement as DrawingElement).rotation || 0}
