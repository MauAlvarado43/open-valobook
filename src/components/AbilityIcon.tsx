...
import { getAbilityDefinition, getMaxDimension, getMinDimension, isFixedSize } from '@/lib/constants/abilityDefinitions';
...
export const AbilityIcon = memo(function AbilityIcon({ element, isSelected, isDraggable, onSelect, onDragEnd }: AbilityIconProps) {
  ...
  const handleResize = (e: Konva.KonvaEventObject<DragEvent>, type: 'radius' | 'wall-start' | 'wall-end' | 'rect-size' | `curved-point-${number}`) => {
    ...
    if (type === 'radius') {
      ...
      const ax = element.points![anchorIdx], ay = element.points![anchorIdx + 1];
      const dx = handlePos.x - ax, dy = handlePos.y - ay;
      ...
      let newX = handlePos.x, newY = handlePos.y;
      ...
      for (let i = 0; i < newPoints.length - 2; i += 2) {
        totalLength += Math.sqrt(Math.pow(newPoints[i + 2] - newPoints[i], 2) + Math.pow(newPoints[i + 3] - newPoints[i + 1], 2));
      }

      const maxDist = getMaxDimension(def, 'width');
      if (totalLength > maxDist) {
        // Simple clamp toward original point if total length exceeds
        const ox = element.points![arrayIndex], oy = element.points![arrayIndex + 1];
        const tx = handlePos.x, ty = handlePos.y;
        let low = 0, high = 1;
        for (let i = 0; i < 10; i++) {
          const mid = (low + high) / 2;
          const testX = ox + (tx - ox) * mid, testY = oy + (ty - oy) * mid;
          const testPoints = [...newPoints];
          testPoints[arrayIndex] = testX; testPoints[arrayIndex + 1] = testY;
          let testLen = 0;
          for (let j = 0; j < testPoints.length - 2; j += 2) {
            testLen += Math.sqrt(Math.pow(testPoints[j + 2] - testPoints[j], 2) + Math.pow(testPoints[j + 3] - testPoints[j + 1], 2));
          }
          if (testLen <= maxDist) low = mid; else high = mid;
        }
  ...
  return (
    <Group
      ...
      id={element.id}
      onMouseEnter={(e) => {
        if (isDraggable && isSelectTool) (e.target.getStage() as Konva.Stage).container().style.cursor = 'move';
      }}
      onMouseLeave={(e) => (e.target.getStage() as Konva.Stage).container().style.cursor = 'default'}
    >
      <AbilityShapeRenderer element={element} isSelected={isSelected} fillColor={fillColor} sideColor={sideColor} />

      {/* Core Icon Box */}
      <Rect
        width={size} height={size} x={-size / 2} y={-size / 2}
        fill="#111827" opacity={element.isGlobal ? 0.4 : 1}
        cornerRadius={6}
      ...
      {status === 'loaded' ? (
        <KonvaImage image={image} width={size - padding * 2} height={size - padding * 2} x={-size / 2 + padding} y={-size / 2 + padding} listening={false} />
      ) : status === 'loading' && (
        <Rect width={size - padding * 2} height={size - padding * 2} x={-size / 2 + padding} y={-size / 2 + padding} fill="#374151" cornerRadius={4} listening={false} />
      )}
      ...
      {isSelected && isSelectTool && (
        <>
          ...
          {(element.subType === 'smoke' || element.subType === 'area') && !isFixedSize(def, 'radius') && (
            <Circle x={element.radius || 60} y={0} radius={8} fill="white" stroke="#3b82f6" strokeWidth={2} draggable
          ...
          {(element.subType === 'wall' || element.subType === 'path') && element.points && (
            <Circle x={element.points[2]} y={element.points[3]} radius={8} fill="white" stroke="#3b82f6" strokeWidth={2} draggable
          ...
          {element.subType === 'curved-wall' && element.points && (
            element.points.slice(2).reduce<JSX.Element[]>((acc, _, i, arr) => {
              if (i % 2 === 0) {
                const idx = (i + 2) / 2;
                acc.push(<Circle key={idx} x={arr[i]} y={arr[i + 1]} radius={7} fill="white" stroke="#3b82f6" strokeWidth={2} draggable
          ...
          {element.subType === 'rect' && (
            <Circle x={element.width ? element.width / 2 : 50} y={element.height ? element.height / 2 : 50} radius={8} fill="white" stroke="#3b82f6" strokeWidth={2} draggable
